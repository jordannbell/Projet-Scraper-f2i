...
// Si vous préférez séparer le CSS, gardez l'import. Sinon, on peut utiliser Tailwind/CSS modules.
// Pour l'instant, on suppose que le CSS fourni sera ajouté globalement ou ici via styled-jsx.
// import './Stepper.css'; 
...
export default function Stepper({
    ...
    return (
        <div className="outer-container" {...rest}>
            <div className={`step-circle-container ${stepCircleContainerClassName}`} style={{ border: '1px solid #e2e8f0', background: 'white' }}>
                    ...
                    <div className={`footer-container ${footerClassName}`}>
                        <div className={`footer-nav ${currentStep !== 1 ? 'spread' : 'end'}`}>
                            ...
                            <button
                                ...
                                {...nextButtonProps}
                                type="button" // Important to prevent form submission default if inside a form
                            >
...
function StepIndicator({ step, currentStep, onClickStep, disableStepIndicators }: any) {
    ...
    return (
        <motion.div onClick={handleClick} className="step-indicator" animate={status} initial={false}>
            <motion.div
                variants={{
                    inactive: { scale: 1, backgroundColor: '#f1f5f9', color: '#94a3b8' }, // Slate-100 / Slate-400
                    active: { scale: 1, backgroundColor: '#6366f1', color: '#ffffff' }, // Indigo-500
                    complete: { scale: 1, backgroundColor: '#6366f1', color: '#ffffff' } // Indigo-500
                }}
...
function StepConnector({ isComplete }: { isComplete: boolean }) {
    const lineVariants = {
        ...
        complete: { width: '100%', backgroundColor: '#6366f1' } // Indigo-500
    };
